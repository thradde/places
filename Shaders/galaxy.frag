...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	/*if(mouse.z > 0.0)
	{
		euler.x = -((mouse.y / resolution.y) * 2.0 - 1.0);
		euler.y = -((mouse.x / resolution.x) * 2.0 - 1.0);
		euler.z = 0.0;
	}*/
	...
	q.y = 0.5 - asin(rd.y)/pi + 0.512 + 0.001*time;
	q *= 2.34;
	...
	float b = 10.0;
	float c = 1.0;
	float fZPos = 5.0;// + time * c + sin(time * a) * b;
	float fSpeed = 0.; //c + a * b * cos(a * time);
