...
//
// June 2015, tr	   - Enabled Unicode
//					   - Provide methods to modify menu items (checked, grayed)
//
...
void CSystemTray::MyLoadMenu()
{
	m_hMainMenu = ::LoadMenu(m_hInstance, MAKEINTRESOURCE(m_tnd.uID));
	if (!m_hMainMenu)
		return;
...
void CSystemTray::CheckMenuItem(UINT uIDItem, bool checked)
{
	::CheckMenuItem(m_hMenu, uIDItem, MF_BYCOMMAND | (checked ? MF_CHECKED : MF_UNCHECKED));
}

void CSystemTray::EnableMenuItem(UINT uIDItem, bool enabled)
{
	::EnableMenuItem(m_hMenu, uIDItem, MF_BYCOMMAND | (enabled ? MF_ENABLED : MF_GRAYED));
}
...
LPCTSTR CSystemTray::GetTooltipText() const
{
    if (!m_bEnabled)
        return FALSE;
...
BOOL CSystemTray::SetMenuDefaultItem(UINT uItem, BOOL bByPos)
{
	...
	::SetMenuDefaultItem(m_hMenu, m_DefaultMenuItemID, m_DefaultMenuItemByPos);
...
LRESULT CSystemTray::OnTrayNotification(WPARAM wParam, LPARAM lParam) 
{
    ...
    {    
        ...
        ::TrackPopupMenu(m_hMenu, 0, pos.x, pos.y, 0, hTargetWnd, NULL);
...
#endif
    {
        ...
        {
            uItem = ::GetMenuItemID(m_hMenu, m_DefaultMenuItemID);
        }
        else
            uItem = m_DefaultMenuItemID;
