...
- VORBEREITEN: gruppe von icons zum reinziehen
	...
	- activation
		...
		- why Places
		- The problem: show desktop with many open windows, now accessing a desktop icon is impossible
		- the hide all windows button is useless (show in video)
		- resize taskbar ==> icons are repositioned, everything is out of shape
		- start menu is cluttered, finding anything there quickly is difficult and takes long time
		- Places runs in the background and can be activated through several different methods while working with other programs
		- it provides its own desktop which consists of ten pages
		- it is resolution independent, if the resolution changes, the icons do not change their positions, they are scaled up or down in size
	==> that is why the program has its name: everything stays always in the same place
		- open programs, files and folders as quick as possible with the least number of user interactions
		- show how places looks after fresh installation (auto german / english)
...
-   Everything stays in its place: icons will always stay in the same position, where you once placed them.
    No sorting, no re-ordering, no changes if the resolution of the screen changes. Instead, the icons are scaled
	smaller or larger as required. You will always find your icons at the same position with a quick mouse click.
    Therefore the name "Places".

-   Eye-Candy and Smooth
    - animated icons
    - animated scrolling between pages
    - bitmaps for icons can be loaded from external files
    - use shaders or images as background wallpaper
    
    - Intuitive and fast user interface
    - Places is moved to front of screen and not hidden by other windows
    ...
    - select all icons on the current page: ctrl-a
    - if a single icon is selected: show its properties with F2
    - page up / down: one page to the left / right
    ...
    - number 1 - 0 (also numpad): jump to n-th page
    - ESC: hide Places
    ...
    - left-click onto selected icon(s): move icon(s)
    ...
    Scrolling
    - left-click and drag mouse left / right to scroll to another page
...
// Folder Blow-Up
// --------------
// Mouse-Hover über Folder: Blow-Up und die darin angezeigten Icons können direkt angeklickt werden
// im Blowup-Mode: Mouse-Wheel scrolled durch die Icons (später)
